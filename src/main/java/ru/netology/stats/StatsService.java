...
public class StatsService {
    ...
    public int sumOfAllSales(int[] sales) {
        int sumMonth = 0; // изначальное значение суммы продаж за все месяцы

        for (int i = 0; i < sales.length; i++) {
            sumMonth = sumMonth + sales[i];
        }

        return sumMonth; // сума значений всех элементов массива
    }

    // #2 Метод возвращающий среднюю сумму продаж в месяц
    public int avrOfAllSales(int[] sales) {
    ...
    public int numberOfMonthWithMaxSales(int[] sales) {
    ...
    public int countOfMonthWithLessAvrSales(int[] sales) {
    ...
    public int countOfMonthWithMoreAvrSales(int[] sales) {
